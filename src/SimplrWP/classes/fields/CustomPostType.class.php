...
/**
 * ## Overview
 * The CustomPostType field is an association to WordPress Custom Post Types in the system
 *
 *
 * ## Examples
 *
 * Here's an example on how you create a CustomPostType field:
 * ```php
 *```
 */
class CustomPostType extends Field {
		
	protected $default_custom_post_type_settings = array(
		'custom_post_type' => false,
		'custom_post_type_types' => array(),
		'allow_multiple' => 1
	);
	
	public function __construct($settings){
		// load defaults
		$this->settings = array_replace_recursive($this->settings, $this->$default_custom_post_type_settings);

		//add ajax filters
		add_action('wp_ajax_simplrwp/fields/custom_post_type/query',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_simplrwp/fields/custom_post_type/query',	array($this, 'ajax_query'));
		...
		if( !empty($field['value']) ) {
			...
			foreach( $field['value'] as $i ) {
				...
				if(sizeof($saved_object_parts)==2) {
					...
					$text_array = array();
					foreach($this->settings['custom_post_type_types'][$object_class] as $current_field) {
